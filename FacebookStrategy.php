...
class FacebookStrategy extends OpauthStrategy{
   ...
   public function int_callback(){
      if (array_key_exists('code', $_GET) && !empty($_GET['code'])){
         ...
         //取得した長期のアクセストークンを配列に格納する
         $resuls['access_token'] = $this->long_access_token($results['access_token']);
         ...
         if (!empty($results) && !empty($results['access_token'])){
            ...
            $this->auth = array(
               ...
               'info' => array(
                  'name' => $me->name,
                  'image' => 'https://graph.facebook.com/'.$me->id.'/picture?width=196&height=196'
               ),
               'credentials' => array(
            ...
            //if (!empty($me->tagged_places)) $this->auth['info']['location'] = $me->tageed_places->name;
   ...
   private function long_access_token($access_token)

   {
      $long_access_token = $this->serverGet('https://graph.facebook.com/oauth/access_token',
         array(
            'client_id' =>$this->strategy['app_id'],
            'client_secret' => $this->strategy['app_secret'],
      ...
      if (!empty($long_access_token)){
         //アクセストークンのみを抽出する
         $long_access_token = preg_replace('/(access_token=)(.*)(&expires=.*)/i', '$2', $long_access_token);
